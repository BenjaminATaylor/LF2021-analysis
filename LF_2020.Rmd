...
gg_exp2_foragingbefore = ggplot(exp2.phenodata.clean, aes(x = as.factor(Rank), y = OffNestBefore, colour = as.factor(Rank))) +
  geom_boxplot(notch = F, outlier.shape = NA, lwd = 1, colour = "black") +
  ggbeeswarm::geom_beeswarm(shape = 19, size = 3, show.legend = FALSE, alpha = .7) +
  ...
  theme(aspect.ratio = 1.2, 
        ...
        strip.text.x = element_text(size=15, face = "bold"),
        strip.background = element_rect(size = 1.5),
        # plot.margin=unit(c(5.5,5.5,30.5,5.5),"points")
        )
...
treatments = c("Rank 1", "Rank 2\n(original rank)", "Rank 2\n(control)", "Rank 2\n(promoted rank)",
           "Rank 3\n(control)", "Rank 3\n(promoted rank)", "Rank 4\n(promoted rank)")
...
gg_exp2_foragingchange = ggplot(subset(exp2.phenodata.clean,Control==F), aes(x = Treatment, y = OffNestChange, colour = Treatment)) +
  ...
  ylab("Change in time off nest") +
  theme_bw() +  
  theme(aspect.ratio = 1, 
        legend.position = "none",
        plot.title = element_text(size=18, face="bold", hjust = 0.5),
        axis.text.x = element_text(size=13, angle = -45, vjust = 0.5, hjust = 0.5, margin = margin(r=1000, t = -10)),
        #axis.text.x = element_text(size=13,angle = 0, vjust = 0),
        axis.text.y = element_text(size=15),
        axis.title.x = element_text(size=18, face = "bold"),
        axis.title.y = element_text(size=18, face = "bold"),
        panel.grid = element_line(size = 0.2, colour = "gray95"),
...
Also try plotting PCA just for ranks 2 to 3 to see how things cluster:

```{r exp2-pca-manipranks}
# treatments_manip = c("Rank 2 (unchanged)", "Rank 2 (control)", "Rank 2 (elevated rank)",
#            "Rank 3 (control)", "Rank 3 (elevated rank)", "Rank 4 (elevated rank)")

exp2.phenodata.clean.manip = subset(exp2.phenodata.clean, Treatment %in% c("R2_S","R2_C","R2_R","R3_C"))
exp2.gene.counts.clean.manip = exp2.gene.counts.clean[,exp2.phenodata.clean.manip$WaspID]

# log-transform with a pseudocount
pca.counts = log2(exp2.gene.counts.clean.manip+1)
#pca.counts = log2(exp2.gene.counts.clean.manip[order(rowVars(exp2.gene.counts.clean.manip),decreasing = T)[1:500],]+1)
#pca.counts = log2(exp2.gene.counts.clean.manip[shiftgenes,]+1) #for this one run shiftgenes code rght at the end of this md
#create pca object
data.pca = prcomp(t(pca.counts),scale. = T)
#extract PC data
percent.var = (data.pca$sdev^2 / sum(data.pca$sdev^2))
pca.out = list(values = data.frame(data.pca$x),
               percent.var = percent.var)
...
ggpcadata = pca.out$values %>% 
  rownames_to_column(var = "WaspID") %>%
  left_join(exp2.phenodata.clean.manip,
            by = "WaspID")
#plot
ggplot(ggpcadata, aes(x = PC1, y = PC2, color = Treatment, label = WaspID)) +
  geom_point(size = 4, position = position_jitter(width = 0.5,height=0.5)) +
  ...
  ylab(paste0("PC",2,": ",signif(pca.out$percent.var[2]*100, 3),"%")) +
  scale_colour_manual(name = "Rank at removal",
                      values = c("midnightblue","midnightblue",brewer.pal(3, "YlOrRd")[c(3,2)]))+
                      #labels = treatments_nonewline) +
  theme_bw() +  
  theme(aspect.ratio = 1, 
        legend.text = element_text(size=15),
...
for(i in 1:nrow(contrasts)){
  ...
  #give comparison object a meaningful name for future reference and save that name in a list
  assign(paste0(contrasts[i,1], ".",contrasts[i,2],".gene.comparison.orig"), comparison)
  gene.comparisons = append(paste0(contrasts[i,1], ".",contrasts[i,2],".gene.comparison.orig"),gene.comparisons)
...
```
Check whether the sets of degs from the two (orig and final rank) comparisons overlap:

```{r}
extractDEGs = function(x){get(x) %>% subset(padj<0.05) %>% row.names()}

newranks = c("1.5.gene.comparison","1.4.gene.comparison","1.3.gene.comparison","1.2.gene.comparison")
newDEGs = sapply(newranks, extractDEGs) %>% unlist() %>% unique()

origranks = c("1.4.gene.comparison.orig","1.3.gene.comparison.orig","1.2.gene.comparison.orig")
origDEGs = sapply(origranks, extractDEGs) %>% unlist() %>% unique()

gene_overlap(newDEGs, origDEGs, background = exp2.gene.counts.clean)
...
SimpleNames = paste0("Module ",c(20:1))
...
gg_exp2_darkturquoisecor = ggplot(data = darkturquoiseframe, aes(x = membership, y = significance)) +
  ...
  ylab("Correlation with foraging effort")+
  annotate(geom="text", x=0.83, y=0, label=darkturquoisecortext,
              color="red", size = 6) +
  theme_bw() +
...
gg_exp2_greencor = ggplot(data = greenframe, aes(x = membership, y = significance)) +
  ...
  xlab("Module Membership") +
  ylab("Correlation with foraging effort")+
  annotate(geom="text", x=0.82, y=0, label=greencortext,
              color="red", size = 6) +
  theme_bw() +
  theme(aspect.ratio = 2/3, 
        legend.text = element_text(size=15),
...
shiftgenes =  unique(c(row.names(subset(R2_R.R3_C.gene.comparison,padj<0.05)),
                       row.names(subset(R2_C.R2_R.gene.comparison,padj<0.05)),
                       row.names(subset(R2_C.R3_C.gene.comparison,padj<0.05))))

#do these genes overlap significantly?
gene_overlap_test(shiftgenes, c(forageDownGenes,forageUpGenes), row.names(exp2.gene.counts.clean))
...
A really interesting result from the above analyses is that direct fitness genes seem to more rather than less derived than indirect fitness genes, which is a reversal of the expected pattern. To try to confirm this result, we'd like to compare our results to more taxa than just P. dominula. As such, we're going to also identify direct and indirect fitness genes in two related vespid species for which caste-biased transcritomic data are available, and we're then going to compare our results to these species'.

# Polistes canadensis

First we import Pcan read counts, which we generated using Nextflow:

```{r import-Pcan}
#import counts
pcan.gene.counts = read.delim("/home/benjamin/Documents/LF_2020_repo/Data_Pcan/merged_gene_counts_Pcan.txt", row.names = NULL)[,-1]
#strip out tRNAs
pcan.gene.counts = pcan.gene.counts[!(substr(pcan.gene.counts$gene_name,1,4)=="Trna"),] %>%
  remove_rownames() %>% column_to_rownames("gene_name") 

#get metadata
metadata = read.csv("/home/benjamin/Documents/LF_2020_repo/Data_Pcan/SRA_metadata.txt", row.names = 1)

#rename sample columns
queenIDs = row.names(metadata)[which(metadata$social_status %in% c("queen",""))]
workerIDs = row.names(metadata)[which(metadata$social_status %in% c("forager"))]
colnames(pcan.gene.counts)[which(substr(colnames(pcan.gene.counts),1,10) %in% queenIDs)] = paste0("queen",1:length(queenIDs))
colnames(pcan.gene.counts)[which(substr(colnames(pcan.gene.counts),1,10) %in% workerIDs)] = paste0("worker",1:length(workerIDs))

#metadata table
pcan.phenodata = data.frame(WaspID = colnames(pcan.gene.counts),
                            caste = stringr::str_sub(colnames(pcan.gene.counts), end = -2))
```

We begin by running a standard differential expression analysis:

```{r DESeq2-Pcan}
DESeq_wrap(data = pcan.phenodata,
           genes = pcan.gene.counts,
           formula = "~ caste",
           detail = F,
           lfc = 1)
```

We then use phylostratigraphy to check whether caste-biased genes in P canadensis exhibit the expected pattern of conservation:
...
```{r import-Vvul}
#import counts
vvul.gene.counts = read.delim("/home/benjamin/Documents/LF_2020_repo/Data_Vvul/merged_gene_counts_Vvul.txt", row.names = 1)[,-1]

#rename columns
queens = which(grepl("Q",colnames(vvul.gene.counts), fixed = T))
workers = which(grepl("W",colnames(vvul.gene.counts), fixed = T))
colnames(vvul.gene.counts)[queens] = paste0("queen",1:length(queens))
colnames(vvul.gene.counts)[workers] = paste0("worker",1:length(workers))
#store caste data
vvul.phenodata = data.frame(WaspID = colnames(vvul.gene.counts),
                            caste = factor(stringr::str_sub(colnames(vvul.gene.counts), end = -2),
                                           levels = c("queen","worker")))

#get all rows for which there are fewer than 1 count/sample total in the group
groups = unique(vvul.phenodata$caste)
cutrows = c()
for(i in 1:length(groups)){
  threshold = nrow(subset(vvul.phenodata, caste == groups[i]))
  subgene = vvul.gene.counts[,which(colnames(vvul.gene.counts) %in% subset(vvul.phenodata, caste == groups[i])$WaspID)]
  cutrows = c(cutrows,which(rowSums(subgene)<threshold))
}
#cut genes that have less than 1 read/sample in every group of interest
remove = as.numeric(names(table(cutrows)[which(table(cutrows) == length(unique(vvul.phenodata$caste)))]))
vvul.gene.counts.clean = as.matrix(vvul.gene.counts[-remove,])
vvul.gene.counts.clean = vvul.gene.counts.clean[,as.character(vvul.phenodata$WaspID)]
```

```{r DESeq2-Vvul}
DEGs_vvul = DESeq_wrap(data = vvul.phenodata,
                       genes = vvul.gene.counts.clean,
                       formula = "~ caste",
                       detail = T)

#genes upregulated in queens
DEGs_vvul_up = DEGs_vvul %>% .$caste_worker_vs_queen %>%
  subset(padj<0.05 & log2FoldChange>0) %>% row.names()

#genes upregulated in workers
DEGs_vvul_down = DEGs_vvul %>% .$caste_worker_vs_queen %>%
  subset(padj<0.05 & log2FoldChange<0) %>% row.names()
```

```{r}
load("/home/benjamin/Documents/LF_2020_repo/Data_Vvul/phylostrata_Vvul.Rdata")

#load gene name reference
vvulIDs = read.csv("/home/benjamin/Documents/LF_2020_repo/Genomic/vvul_gene_names.csv", header = F, row.names = NULL)  %>%
  unique() %>% 'colnames<-'(c("GTF_ID","FASTA_ID"))

unique(phylostrata[,c("ps","mrca_name")])

DEGs_vvul_upphylo = vvulIDs$GTF_ID[which(vvulIDs$FASTA_ID %in% DEGs_vvul_up)]
DEGs_vvul_downphylo = vvulIDs$GTF_ID[which(vvulIDs$FASTA_ID %in% DEGs_vvul_down)]
...
# divide into groups based on correlation with foraging rate
phylo_up = subset(phylostrata, qseqid %in% DEGs_vvul_upphylo)$bin
phylo_down = subset(phylostrata, qseqid %in% DEGs_vvul_downphylo)$bin
phylo_neutral = subset(phylostrata, !(qseqid %in% c(DEGs_vvul_upphylo, DEGs_vvul_downphylo)))$bin


#plot
strataframe_vvul =  rbind(data.frame(treatment = "up",strata = phylo_up),
                     data.frame(treatment = "down",strata = phylo_down),
                     data.frame(treatment = "neutral",strata = phylo_neutral)) %>%
  # mutate(strata = factor(strata,levels = c("Lineage-specific","Hymenoptera","Arthropoda","Metazoa","Ancient"))) %>%
  mutate(strata = factor(strata,levels = c("Aculeata","Hymenoptera","Insecta","Ancient"))) %>%
...
strataplot_vvul = ggplot(strataframe_vvul, aes(x = treatment, fill = strata)) +
  geom_bar(position = "fill", width = 0.9, colour = "black",) +
  #scale_fill_brewer(palette = "Blues",direction = -1)+
  scale_fill_manual(values = brewer.pal(6,"Blues")[3:6], guide = guide_legend(reverse=T)) +
  scale_x_discrete(labels = c("Queen\nbiased","Worker\nbiased","Unbiased"), 
...
For Max: what are genes that are up/downregulated with foraging rate among non-reproductives doing in Rank 1? Max hypothesises that these genes might be for e.g. 'nursing' behaviour, in which case they would rise in expression as we reach rank 2, but then suddenly drop off in rank 1. Actually, that's not what we find:

```{r}
#get set of genes that are negatively associated with foraging rate among non-reproductives
testOffNest = DESeq_wrap(data = exp2.phenodata.clean.no1.resids,
                                       genes = exp2.gene.counts.clean,
                                       formula = "~ OffNestFinal",
                                       detail = T)

foo = subset(testOffNest$OffNestFinal, padj<0.05 & log2FoldChange<0)
nrow(foo)
laborDownGenes = row.names(foo)

bar = subset(testOffNest$OffNestFinal, padj<0.05 & log2FoldChange>0)
nrow(bar)
laborUpGenes = row.names(bar)

#what is mean expression of these genes in rank 4?
all4s = subset(exp2.phenodata.clean, Rank == 4)$WaspID
all3s = subset(exp2.phenodata.clean, Rank == 3)$WaspID
all2s = subset(exp2.phenodata.clean, Rank == 2)$WaspID
all1s = subset(exp2.phenodata.clean, Rank == 1)$WaspID

#genes upregulated with labor effort - yes, they reach lowest expression in rank 1
mean(exp2.gene.counts.clean[laborUpGenes,all4s])
mean(exp2.gene.counts.clean[laborUpGenes,all3s])
mean(exp2.gene.counts.clean[laborUpGenes,all2s])
mean(exp2.gene.counts.clean[laborUpGenes,all1s])

#genes downregulated with labor effort - yes, they reach highest expression in rank 1
mean(exp2.gene.counts.clean[laborDownGenes,all4s])
mean(exp2.gene.counts.clean[laborDownGenes,all3s])
mean(exp2.gene.counts.clean[laborDownGenes,all2s])
mean(exp2.gene.counts.clean[laborDownGenes,all1s])
```
