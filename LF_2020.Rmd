...
gg_exp2_upDEG_overlaps = ggplot(data = comps_plot, aes(x=Var1, y=Var2, fill=value)) + 
  ...
  scale_y_discrete(labels = c("Reroductives", "Time on Nest")) +
  geom_tile(color = "black") +
...
gg_exp2_downDEG_overlaps = ggplot(data = comps_plot, aes(x=Var1, y=Var2, fill=value)) + 
  ...
  geom_text(aes(label=na.omit(c(labs_down))),
            nudge_y = -0.15, size = 3) +
  scale_x_discrete(labels = c("Foraging effort","Polistes dominula\nworkers")) +
  scale_y_discrete(labels = c("Non-reproductives","Foraging effort")) +
  scale_fill_gradientn(colours = colorRampPalette(rev(c("#FFFFFF",brewer.pal(n = 9, name = "Reds")[1:5])),bias=6)(20),
                       breaks = c(0.0,0.05,1),
                       expand = c(0,0),
                       limits = c(0,1),
...
stratacontingency_ancient = cbind(stratacontingency[,1],rowSums(stratacontingency[,2:4]))
...
phylo_neutral = subset(phylostrata, !(qseqid %in% c(genes_down_exp2_rank, genes_up_exp2_rank)))$bin
