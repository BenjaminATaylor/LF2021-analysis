...
makeblastdb -in species_fastas/Vespa_vulgaris.faa -dbtype 'prot' -out databases/Vvul_db

# run BLAST search
# -evalue: equivalent to significance value (lower values = more stringent matching)
# -outfmt: out put format (6 = tabular)
blastp -num_threads 8 -evalue 1e-6 -outfmt 6 -query species_fastas/Liostenogaster_flavolineata.faa -db databases/Vvul_db > LF_to_Vvul